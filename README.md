...
1. Download this repo and put it at /bundles/decoy
2. Install all the bundle dependencies:
...
3. Initialize Decoy in the bundles.php file with: `'decoy' => array('auto' => true, 'handles' => 'admin'),`
4. To customize the default user credentialt, edit the `default_login` and `default_password` fields within the config file (/config/decoy.php)
5. Run a general migration to create the standard decoy databae tables and generate the default user.
6. Make all admin controllers and models extend from Decoy_Base_Controller and Decoy_Base_Model.  I generally have an application Base_Controller and Base_Model for the front end, both of which inherit from the Decoy classes.
7. Configure Compass to look for sass in the decoy bundle by adding the following to the compass config.rb file: `add_import_path "bundles/decoy/sass"`.  Then, within the sass file for your application's admin area, include the decoy sass with a simple: `@import "decoy";`.  Here's [an example](https://gist.github.com/39230d1241590093986e).
